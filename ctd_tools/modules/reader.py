...
class SBEAsciiReader:
    ...
    def __parse_data(self, file_path):
        ...
        for line in lines[data_start:]:
            ...
            if len(parts) == 4:  # Case without pressure
                ...
                timestamp = datetime.datetime.strptime(f"{date} {time}", "%d %b %Y %H:%M:%S")
    ...
    def __create_xarray_dataset(self, df, metadata, sample_interval):
        ds = xr.Dataset.from_dataframe(df)
        ...
        ds.attrs["source"] = "Sea-Bird SBE37"
    ...
    def __read(self):
        ...
        ds = self.__create_xarray_dataset(df, metadata, sample_interval)
        self.data = ds
